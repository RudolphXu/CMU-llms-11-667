...
for idx, file in enumerate(current_path_list):
    if file == 'src':
        break
sys.path.insert(0, '/'.join(current_path_list[:idx+1]))
...
class EncoderModel(nn.Module):
    ...
    def pooling(self, 
        ...
        # Find the index of the last token for each sequence using the attention mask
        token_lens = attention_mask.sum(dim=1) - 1  # Shape: (batch_size,)
        
        # Gather the last token's representation for each sequence
        batch_size, seq_len, hidden_dim = last_hidden_state.shape
        indices = token_lens.unsqueeze(1).unsqueeze(2).expand(batch_size, 1, hidden_dim)
        last_token_reps = torch.gather(last_hidden_state, 
                                       dim = 1, 
                                       index = indices).squeeze(1)  # Shape: (batch_size, hidden_dim)
        
        # Apply L2 normalization to the embeddings
        reps = F.normalize(last_token_reps, 
                           p = 2, 
                           dim = 1
    ...
    def compute_similarity(self, 
        ...
        similarity_matrix = torch.matmul(q_reps, p_reps.T)
    ...
    def compute_labels(self, 
        ...
        #raise NotImplementedError()
        # Compute the number of passages per query
        passages_per_query = n_passages // n_queries
        
        # Generate labels where the first passage of each group is positive
        target = torch.arange(0, n_passages, passages_per_query)
