...
def read_data(seed: int, 
              train_size: int, 
              test_size: int) -> Tuple[List[str], List[int], List[str], List[int]]:
    """
...
def get_responses(prompts: List[str], echo: bool = False) -> List[Any]:
    ...
    client = openai.OpenAI(
        api_key = os.getenv("OPENAI_API_KEY"),  # please use .env file to store your key
...
def calibrate(
    ...
    p_y = np.exp(p_y - np.max(p_y))  # Prevent overflow
    p_y /= np.sum(p_y)  # Normalize to sum to 1
